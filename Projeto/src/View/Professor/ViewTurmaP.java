...
public class ViewTurmaP extends JPanel {
	...
	public ViewTurmaP(Turma t, ProfessorView pv) {
		...
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.LEADING)
				.addGroup(groupLayout.createSequentialGroup().addContainerGap()
						...
						.addComponent(scrollPane, GroupLayout.DEFAULT_SIZE, 460, Short.MAX_VALUE).addContainerGap()));

		table = new JTable();
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				pv.setViewPanel(
						new ViewStudentP(new Student((String) table.getValueAt(table.getSelectedRow(), 0)), pv));
			}
		});
	...
	private void info() {
		String read = "select count(distinct a.matricula), count(distinct s.id) from estuda es join turmas t "
	...
	private void listar() {
		DefaultTableModel model = new DefaultTableModel(new Object[][] {},
				new String[] { "Matricula", "Aluno", "Solicitações" }) {
			private static final long serialVersionUID = 1L;
		...
		String readStudents = "select a.matricula, a.nome, count(s.id) from alunos a left join solicitacoes s "
				+ "on s.matricula_a = a.matricula and s.matricula_p= '" + pv.prof.getMatricula() + "' "
				+ "where a.id_turma= '" + t.getId() + "' group by a.matricula order by a.nome";
