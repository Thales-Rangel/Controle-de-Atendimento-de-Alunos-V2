...
public class ViewTurmasListP extends JPanel {
	...
	public ViewTurmasListP(Professor p, ProfessorView pv) {
		...
		groupLayout.setHorizontalGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(groupLayout
				...
				.addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
						...
						.addGroup(groupLayout.createSequentialGroup()
								.addComponent(lblTitle, GroupLayout.DEFAULT_SIZE, 204, Short.MAX_VALUE).addGap(75)
								...
								.addGroup(groupLayout.createParallelGroup(Alignment.TRAILING, false)
										.addGroup(Alignment.LEADING,
												groupLayout.createParallelGroup(Alignment.BASELINE)
														.addComponent(textField, GroupLayout.DEFAULT_SIZE, 26,
																Short.MAX_VALUE)
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				pv.setViewPanel(new ViewTurmaP(new Turma((int) table.getValueAt(table.getSelectedRow(), 0),
						(String) table.getValueAt(table.getSelectedRow(), 1)), pv));
			}
		});
	...
	private void buscar() {
		DefaultTableModel model = new DefaultTableModel(new Object[][] {},
				new String[] { "ID", "Turma", "Qnt. Alunos", "Solicitações" }) {
			private static final long serialVersionUID = 1L;
		...
		String read = "select t.*, count(distinct a.matricula), count(distinct s.id) from estuda es join turmas t "
				+ "on t.id = es.id_turma join ensina en on en.id_disciplina = es.id_disciplina "
				+ "left join alunos a on a.id_turma = t.id left join solicitacoes s "
				+ "on s.matricula_a = a.matricula and en.matricula_professor = s.matricula_p "
				+ "where en.matricula_professor= '" + p.getMatricula() + "' and t.nome like '"
				+ textField.getText().trim() + "%' group by t.id order by t.nome";
